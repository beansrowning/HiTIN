from hi_tin import Vocab, HiAGM, data_loaders, Configure
import transformers
import torch
import os
import numpy as np
...
class Predictor(object):
    def __init__(self, config: Configure, checkpoint_type: str = "macro"):
        ...
        if config.text_encoder.type == "bert":
            self.tokenizer = transformers.BertTokenizer.from_pretrained(config.text_encoder.bert_model_dir)
        else:
            self.tokenizer = None
        
        # Load in data
        self.data_loaders = {k: v for k, v in zip(["train", "validation", "test"], data_loaders(config, self.vocab, tokenizer=self.tokenizer))}
        
        # Load in latest checkpoint data
        latest_checkpoint = self.get_model_checkpoint(config.train.checkpoint.dir, checkpoint_type)
        print(f"Loading pre-trained model weights from {latest_checkpoint}...")
        self.model_chkpt = torch.load(latest_checkpoint)

        # Init model
        self.model = HiAGM(config, self.vocab, model_type=config.model.type, model_mode='EVAL')
        self.model.load_state_dict(self.model_chkpt["state_dict"])
        self.model.to(self.device)

    @staticmethod
    def get_model_checkpoint(ckpt_dir: os.PathLike, type: str = "macro") -> os.PathLike:
        """
        ...
        if not os.path.isdir(ckpt_dir):
            ...
            file_list = [file.path for file in os.scandir(dir_list[0]) if file.name.startswith(f"best_{type}")]

        return(file_list[0])

    @staticmethod
    def optimize_threshold(y_true: ArrayLike, y_pred: ArrayLike) -> float:
        """
    ...
    def predict(self, dataset:str = "test", optimized_thresholds: bool = True) -> tuple[list[list[int]], list[list[int]]]:
        """
    ...
    def compute_label_thresholds(self):
        ...
        for i, pred_proba in enumerate(y_pred.T):
            ...
            thresh = self.optimize_threshold(y_true=true, y_pred=pred_proba)
            cut_points.append(thresh)
        
        self.lab_thresholds = cut_points
        self.lab_thresholds_dict = {self.vocab.i2v["doc_label"][i]: v for i, v in enumerate(cut_points)}
